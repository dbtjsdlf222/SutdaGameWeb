...
public class RegularChecker {
	...
	public String join(PlayerVO vo, String passwordc) {
		if (id(vo.getId())) {
			return "아이디";
	...
	private boolean pw(String pw, String pwc) {
		try {
			if (!Pattern.matches("/(?=.*\\d{1,20})(?=.*[~`!@#$%\\^&*()-+=]{1,20})(?=.*[a-zA-Z]{1,20}).{8,20}$/", pw)) {
				return false;
			} else if (pw == "" || pw == null) {
				return false;
	...
	private boolean email(String email) {
		try {
			...
			if (!Pattern.matches("/^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/i",
					email)) {
				return false;
